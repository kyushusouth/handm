...
class MetricsCalculator:
    ...
    def calc(self, eval_df: pd.DataFrame, true_items_col: str, pred_items_col: str) -> Metrics:
        """評価指標の計算"""
        metrics_df = eval_df.apply(
            lambda row: pd.Series(
                {
                    "precision": self.precision_at_k(row[true_items_col], row[pred_items_col]),
                    "recall": self.recall_at_k(row[true_items_col], row[pred_items_col]),
        ...
        return Metrics(**metrics_df.mean())

    def precision_at_k(self, true_items: List[int], pred_items: List[int]) -> float:
        if len(pred_items) == 0:
            return 0
        precision_at_k = (len(set(true_items) & set(pred_items))) / len(pred_items)
        return precision_at_k

    def recall_at_k(self, true_items: List[int], pred_items: List[int]) -> float:
        if len(true_items) == 0:
            return 0
        recall_at_k = (len(set(true_items) & set(pred_items))) / len(true_items)
        return recall_at_k

    def f1_at_k(self, true_items: List[int], pred_items: List[int]) -> float:
    ...
    def rr_at_k(self, true_items: List[int], pred_items: List[int]) -> float:
        rr = 0
    ...
    def get_user_relevance(self, true_items: List[int], pred_items: List[int]) -> List[int]:
    ...
    def ap_at_k(self, true_items: List[int], pred_items: List[int]) -> float:
        ...
        return sum([sum(user_relevances[: idx + 1]) / (idx + 1) for idx in nonzero_indices]) / sum(user_relevances)
